...
public class TicketPDFExporter {
    ...
    public TicketPDFExporter(Ticket ticket) {

        ticketId = ticket.getId();
        valid = ticket.isValid();
        trainName = ticket.getTrain().getTrainName();
        stationFrom = ticket.getStationFrom().getTitle();
        stationTo = ticket.getStationTo().getTitle();
        departureTime = sdf.format(ticket.getDepartureTime());
        arrivalTime = sdf.format(ticket.getArrivalTime());

        firstName = ticket.getPassenger().getFirstName();
        lastName = ticket.getPassenger().getLastName();
        birthdate = sdf1.format(ticket.getPassenger().getBirthDate());
    }
